...
class LoginFilterMvcTest {
  ...
  @Autowired
  PasswordEncoder encoder;  // because AuthenticationManager uses declared PasswordEncoder to compare password, it is necessary to encode password when mocking UserDetailsService
  @Autowired
  ObjectMapper mapper;
  @Autowired
  ...
  void shouldReturnSuccessfulResponse() throws Exception {
    ...
    doNothing().when(tokenService).login(anyString(), anyString(), anyString());
    ...
    mvc
      ...
      .andExpect(jsonPath("$.key", not(emptyString())));
  }
}
